...
public class SimpleUtils<T> {
    ...
    public static <T extends Comparable<T>> T least(Collection<T> items, boolean from_start){
        return items.stream().reduce((x,y) -> x.compareTo(y) <= 0 ? (from_start ? x : y) : y).get();
    }
    /**
    ...
    public static <K, V> List<String> flatten(Map<K, V> aMap){
        return aMap.entrySet().stream().map(x -> x.getKey().toString() + " -> " + x.getValue().toString()).collect(Collectors.toList());
    }
}